...
export class DashboardController {
  ...
  async getSummary(req, res, next) {
    try {
      console.log(
        `[Trace Log - Controller] GET /api/v1/dashboard/summary for user ${req.user.id} (${req.user.role}). Date range: ${req.query.startDate} to ${req.query.endDate}`
      );
  ...
  async getActivityLogs(req, res, next) {
    try {
      console.log(
        `[Trace Log - Controller] GET /api/v1/dashboard/activity-logs for user ${req.user.id} (${req.user.role})`
      );
      const logs = await dashboardService.getActivityLogs(req.user.id, req.user.role);
      console.log(
        `[Trace Log - Controller] GET /api/v1/dashboard/activity-logs success. Found ${logs.length} logs.`
      );
