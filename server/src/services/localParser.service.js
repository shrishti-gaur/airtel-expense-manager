...
export class LocalParserService {
  ...
  parse(rawText) {
    ...
    if (category === 'Meals') {
      ...
      // Extract PNR
      const pnrMatch = cleanText.match(
        /\b(?:pnr|booking\s*(?:reference|ref)|pnr\s*no)[:\s-]*([A-Z0-9]{6})\b/i
      );
      ...
      category === 'Utilities' &&
      /(?:petrol|diesel|fuel|litre|ltr|pump)/i.test(cleanText)
    ) {
      ...
      if (litres > 0 && rate > 0) {
        ...
        if (diff < 5.0) {
          console.log(
            `[Local Parser] Fuel validation passed! ${litres} Ltrs @ ₹${rate} = ₹${calculatedAmount.toFixed(2)} (Diff: ${diff.toFixed(2)})`
          );
          amountConfidence = 0.95;
        } else if (amountRes.value === 0 || amountRes.confidence < 0.7) {
          // Fallback to calculated amount
          console.log(
            `[Local Parser] Overwriting amount with validated fuel product sum: ₹${calculatedAmount.toFixed(2)}`
          );
          amountRes.value = calculatedAmount;
          amountConfidence = 0.9;
        } else {
          console.warn(
            `[Local Parser] Fuel mathematical validation failed! Calculated: ${calculatedAmount.toFixed(2)}, Extracted: ${amountRes.value}`
          );
      ...
      const accMatch = cleanText.match(
        /(?:account\s*no|subscriber\s*(?:no|number)|tel\s*no|mobile\s*no|phone\s*no)[:\s-]*(\d{8,12})/i
      );
    ...
    if (finalAmount > 0 && finalGst > finalAmount) {
      console.warn(
        `[Local Parser] Mathematical validation failed: GST (${finalGst}) is greater than Amount (${finalAmount})`
      );
    ...
    if (finalCgst > 0 && finalSgst > 0 && Math.abs(finalCgst - finalSgst) > 1.0) {
      console.warn(
        `[Local Parser] Mathematical validation failed: CGST (${finalCgst}) does not match SGST (${finalSgst})`
      );
      isMathInconsistent = true;
    }
    ...
    if (finalIgst > 0 && (finalCgst > 0 || finalSgst > 0)) {
      console.warn(
        `[Local Parser] Mathematical validation failed: Cannot have both IGST (${finalIgst}) and CGST/SGST (${finalCgst}/${finalSgst})`
      );
      isMathInconsistent = true;
    }
    ...
    if (category !== 'Utilities' && finalSubtotal > 0 && finalAmount > 0) {
      ...
      if (difference > 1.0) {
        console.warn(
          `[Local Parser] Mathematical validation failed! Subtotal (${finalSubtotal}) + GST (${finalGst}) - Discount (${finalDiscount}) = ${calculatedTotal}, but Grand Total is ${finalAmount}. Diff: ${difference}`
        );
        isMathInconsistent = true;
      }
    }
    ...
    if (isMathInconsistent) {
      ...
      console.log(
        `[Local Parser] Mathematical validation passed! Subtotal (${finalSubtotal}) + GST (${finalGst}) - Discount (${finalDiscount}) matches Grand Total (${finalAmount}).`
      );
    }
    ...
    for (let i = 0; i < Math.min(lines.length, 5); i++) {
      ...
      if (!matchesGeneric && !matchesExclude) {
        const cleanedVal = line
          .replace(/(?:\s+stay)?\s+(?:receipt|invoice|bill|challan|slip|payment)$/i, '')
          .trim();
        return {
          value: cleanedVal,
          confidence: 0.7,
        ...
        for (const token of tokens) {
          if (/[A-Z]+[-/]\d+/.test(token) || /\d+[-/][A-Z\d]+/.test(token)) {
            ...
            if (cleanToken.length >= 3 && cleanToken.length < 20) {
              return { value: cleanToken, confidence: 0.8 };
            }
          }
        }
      }
  ...
  extractInvoiceDateWithConfidence(rawText) {
    ...
    if (labelMatch && labelMatch[1]) {
      ...
      if (parsedDate) {
        return { value: parsedDate, confidence: 0.9 };
      }
    }
    ...
    if (parsedDate) {
      return { value: parsedDate, confidence: 0.6 };
    }
  ...
  parseAndNormalizeDate(text) {
    ...
    const textMonthRegex1 = new RegExp(`\\b(\\d{1,2})\\s+(${monthNames})\\s+(\\d{4})\\b`, 'i');
    ...
    const textMonthRegex2 = new RegExp(`\\b(${monthNames})\\s+(\\d{1,2})\\,?\\s+(\\d{4})\\b`, 'i');
  ...
  extractTaxAndSubtotalWithConfidence(rawText) {
    ...
    for (const line of lines) {
      ...
      if (gstKeywords.test(line)) {
        ...
        if (
          /(?:gstin|gst\s*(?:no|number|reg|id)|pan|phone|mobile|tel|email|address|website|date|dated|time|invoice|bill|receipt|txn|transaction|challan|taxable)/i.test(
            line
          )
        ) {
          continue;
  ...
  extractAmountWithConfidence(rawText, existingTaxes = []) {
    const totalKeywords =
      /(?:total|grand\s*total|net\s*payable|amount\s*due|amount\s*paid|total\s*payable|total\s*amount|gross\s*amount|payable)/i;
    ...
    for (const line of lines) {
      if (totalKeywords.test(line)) {
        ...
        if (val !== null && val > 0) {
          ...
          else if (lowerLine.includes('net payable') || lowerLine.includes('total payable'))
            score += 4;
          else if (
            lowerLine.includes('total amount') ||
            lowerLine.includes('amount paid') ||
