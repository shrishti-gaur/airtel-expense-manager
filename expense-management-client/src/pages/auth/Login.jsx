...
const Login = () => {
  const { login, loginSimulated } = useAuth();
  ...
  const handleSimulatedLogin = (role) => {
    const user = loginSimulated(role);
    redirectUser(user.role);
  };
    ...
    <div className="flex min-h-screen w-screen items-center justify-center bg-slate-900 px-4 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-8 shadow-2xl backdrop-blur-md">
          ...
          {/* Simulated Credentials Drawer */}
          <div className="mt-8 border-t border-slate-800 pt-6">
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-3 text-center">
              Testing Environment Quick Logins
            </p>
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={() => handleSimulatedLogin('Employee')}
                className="rounded-lg border border-slate-800 bg-slate-900/40 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white cursor-pointer transition-colors"
              >
                Employee
              </button>
              <button
                type="button"
                onClick={() => handleSimulatedLogin('Manager')}
                className="rounded-lg border border-slate-800 bg-slate-900/40 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white cursor-pointer transition-colors"
              >
                Manager
              </button>
              <button
                type="button"
                onClick={() => handleSimulatedLogin('Finance')}
                className="rounded-lg border border-slate-800 bg-slate-900/40 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white cursor-pointer transition-colors"
              >
                Finance
              </button>
            ...
            <p className="mt-4 text-[10px] text-center text-slate-500 italic">
              Active Entra ID JWT verification is bypassable via role emulation for layout review.
            </p>
          </div>
        </div>
      </div>
